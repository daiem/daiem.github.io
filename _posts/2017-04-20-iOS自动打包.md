...
导出`.ipa`包经常会出现错误，~~在ruem2.4.0版本中会报错，所以请使用其他版本的ruem~~,最初的原因是使用了 ruem2.4.0 进行编译时出现的错误。

解决方法是低版本的 ruem 进行编译，如使用系统版本：`rvm use system`。后面升级macOS系统（10.12.5）后发现 ruem2.4.0 能成功 导出ipa包了。
...
~~再次提醒，请不要使用 ruem 2.4.0 运行该脚本！~~，若在 ruem 2.4.0 下编译失败，请切换低版本的ruem。
...
GitHub：<https://github.com/daiem/iOSAutoArchiveScript>
